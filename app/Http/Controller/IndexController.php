...
class indexController extends Controllers
{
    ...
    /* 网站自助充值 已弃用
    public function adminapichongzhi()
    {
        ...
        if ($user_info) {
            ...
            //修改数据
            $info = [$user_info['u_name'], $user_info['u_email'], $balance, $user_info['u_id']];
            $res = self::$db->exec(ADMIN_USER_EDIT, $info); //修改余额
            if ($res) {
                // 日志信息
                $log = [$user_info['u_name'], $price, $price, 5,  $payno, "网址在线充值", $time, 2, $time];
                $res = self::$db->exec(WEB_USER_ADDFUNDS, [$log]); //添加加款记录
                // 返回处理结果
                if ($res) {
                    return response_tips(["code" => "0", 'msg' => "充值成功"]);
                } else {
                    return response_tips(["code" => "203", 'msg' => "记录失败"]);
                }
            } else {
                return response_tips(['code' => '400', 'msg' => '充值失败']);
            }
        } else {
            return response_tips(['code' => '412', 'msg' => '用户不存在！']);
        }
    }*/
    //前台首页
    ...
    public function api()
    {
        ...
        // } else {
        //     $data = self::$db->fetchRow(API_INFO3, [$apiVal]); //接口基本信息
        ...
        if ($data) {
            ...
            if ($data['l_pay'] == 1) {
                // 验证密钥有效性
                $ow = self::$db->fetchRow(IS_KEY, [$data['l_id'], $apiKey]);
                if (!$ow) {
                    exit("您提交的密匙不正确！付费接口请充值购买");
                }
            }
            //判断接口是否正常开放
            if ($data['l_show'] == 1) {
                ...
                if ($apiKey != "") {
                    self::apiPersonalCount($ow['ow_id'], $ow['ow_count']);
                }
                //判断是否为数据类型接口
                if ($data['l_data']) {
                    //获取数据
    ...
    public function statisticsVisitCount()
    {
        ...
        // 今日调用计数
        $one_day_data['count'] = ($online_day_log ? $online_day_log : 0) + 1;
        //更新今日计数
        Storage::hold_all(WWW_CACHE_PATH . $one_day . '.php', $one_day_data);
        // 获取总数
        $online_log = config('count', 'count');
        $data['count'] = ($online_log ? $online_log : 0) + 1;
