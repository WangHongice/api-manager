...
class HandleController extends Controllers
{
    ...
    public function ownededit()
    {
        ...
        if (!empty($val['id'])) {
            //查询账号
            $data = self::$db->fetchRow(ADMIN_OWNED_ROW, [$val['id']]);
            if ($data > 0) {
                //修改数据
                $res = self::$db->exec(ADMIN_OWNED_EDIT, [$val['lid'], $val['time'], $val['id']]); //修改已购买接口
                //判断修改是否成功
                if ($res > 0) {
    ...
    public function addkami()
    {
        Authentication::AdminAuth(false);
        $val = Request::post();
        $val_data = ['money', "count"];
        if (is_empty($val_data, $val)) {
            $sql = "INSERT INTO api_kami (`a_k_content`, `a_k_money`, `a_k_state`) VALUES ";
            $txt = '';
            // 循环写入卡密
            for ($i = 0; $i < $val['count']; $i++) {
                $kami = setKami();
                $sql .= "('".$kami."', '".$val['money']."',1),";
                $txt .= "<br>".$kami;
            }
            self::$db->query(rtrim($sql, ','));
            return response_tips(['code' => '200', 'msg' => '添加成功'.$txt]);
        } else {
            return response_tips(['code' => '412', 'msg' => '传递参数错误']);
        }
    }
    ...
    //清理未验证的用户
    public function delunverify()
    {
        Authentication::AdminAuth(false);
        $val = Request::post();
        
        if (!empty($val['date'])) {
            $data = self::$db->fetchRow('SELECT * FROM `api_user` WHERE `a_u_state` = 0 AND `a_u_found_time` < ?', [$val['date']]);
    ...
    public function loginuser()
    {
        ...
        //判断信息是否为空
        if ($data['u_state'] == 0) {
            return response_tips(["code" => "204", 'msg' => "当前账号未通过邮箱验证~"]);
        } else {
            //记录登录状态
    ...
    public function setuser()
    {
        ...
        $val = Request::post();
        $val = $val ? $val : ['verify' => ""];
        if (Storage::hold_all(APP_CONFIG . 'user.php', $val) > 0) {
            return response_tips(['code' => '200', 'msg' => '保存成功']);
        } else {
            return response_tips(['code' => '204', 'msg' => '保存失败']);
        }
        ...
        if (is_empty($val_name, $val)) {
            ...
            if ($data > 0) {
                ...
                if ($res > 0) {
                    ...
                    $log = [$val['user'], $val['balance'], $val['balance'], 0,  $date, $bz, time(), 2, time()];
