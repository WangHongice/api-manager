...
define("ADMIN_USERS", "SELECT api_user.a_u_id AS u_id,a_u_name AS u_name,a_u_passwd AS u_passwd,a_u_email AS u_email,a_u_balance AS u_balance,a_u_found_time AS u_found_time,a_u_state AS u_state FROM api_user LEFT JOIN api_user_verify ON api_user.a_u_id = api_user_verify.a_u_id");

//用户详情
define("ADMIN_USER_ROW", "SELECT a_u_id AS u_id,a_u_name AS u_name,a_u_passwd AS u_passwd,a_u_email AS u_email,a_u_balance AS u_balance,a_u_found_time AS u_found_time,a_u_state AS u_state FROM api_user WHERE a_u_id = ?");

//用户修改
define("ADMIN_USER_EDIT", "UPDATE `api_user` SET a_u_name= ?,a_u_email=? ,a_u_balance=? ,a_u_state=? WHERE a_u_id= ?;");

//用户封禁
define("ADMIN_USER_BAN", "UPDATE `api_user` SET a_u_state=? WHERE a_u_id= ?;");
...
//会员人数
define('WEB_USER', "SELECT count(*) FROM `api_user`");

//当天注册人数
define('WEB_USER_DAY', "SELECT count(*) FROM `api_user` WHERE date_format(`a_u_found_time`,'%Y-%m-%d')= ?");

//充值金额
define('WEB_USER_PAY', "SELECT * FROM `codepay_order` WHERE date_format(`up_time`,'%Y-%m-%d')= ?");

//用户消费
define('USER_xPAY', "SELECT * FROM `api_order` WHERE o_u_id=?");

//接口数量
define('WEB_USER_API', "SELECT count(*) FROM `api_info`");
...
//已购买接口详情
define("ADMIN_OWNED_ROW","SELECT * FROM `api_owned` WHERE `ow_id` = ?");

//已购买接口修改
define("ADMIN_OWNED_EDIT", "UPDATE `api_owned` SET ow_l_id=? ,ow_end_time=? WHERE `ow_id` = ?;");