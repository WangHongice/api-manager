...
//用户修改余额
define("USER_BALANCE", "UPDATE api_user SET a_u_balance=? WHERE a_u_id= ?;");

//用户注册
define('USER_REGISTER', "INSERT INTO api_user (a_u_name,a_u_passwd,a_u_email,a_u_balance,a_u_state) VALUES (?,?,?,?,?);");

//用户查询
define('USER_LOGIN', "SELECT a_u_id AS u_id,a_u_name AS u_name,a_u_passwd AS u_passwd,a_u_email AS u_email,a_u_balance AS u_balance,a_u_found_time AS u_found_time,a_u_state AS u_state FROM api_user WHERE a_u_name= ?");

// 用户查询->验证版
define('USER_LOGIN_VERIFY', "SELECT api_user.a_u_id AS u_id, a_u_name AS u_name, a_u_passwd AS u_passwd, a_u_email AS u_email, a_u_balance AS u_balance, a_u_found_time AS u_found_time, a_u_state AS u_state,a_u_state AS u_state FROM api_user WHERE a_u_name = ?");
...
//用户查询 id
define('USER_LOGIN_ID', "SELECT a_u_id AS u_id,a_u_name AS u_name,a_u_passwd AS u_passwd,a_u_email AS u_email,a_u_balance AS u_balance,a_u_found_time AS u_found_time,a_u_state AS u_state FROM api_user WHERE a_u_id= ?");
...
//api通道信息3
define('API_INFO3', "SELECT a_l_id AS l_id,a_l_title AS l_title,a_l_desc AS l_desc,a_l_alias AS l_alias,a_l_address AS l_address,a_l_format AS l_format,a_l_mode AS l_mode,a_l_ask AS l_ask,a_l_demo AS l_demo,a_l_example AS l_example,a_l_data AS l_data ,a_l_show AS l_show ,a_l_found_time AS l_found_time,a_l_count AS l_count,a_l_pay AS l_pay,api_owned.* FROM api_info LEFT JOIN api_owned ON api_info.a_l_id = api_owned.ow_l_id WHERE api_info.a_l_alias = ?;"); //LIMIT 0, 10000;
...
// 查询卡密信息
define("GET_KAMI", "SELECT * FROM api_kami WHERE a_k_content = ?;");

// 更新卡密状态
define("SET_KAMI", "UPDATE api_kami SET a_k_state = ? , a_u_name = ? WHERE a_k_id = ?;");

// 验证密钥
define("IS_KEY", "SELECT * FROM api_owned WHERE ow_l_id = ? AND ow_md5 = ?;");
...
// 删除验证
define("USER_STATE", "UPDATE `api_user` SET `a_u_state` = ? WHERE `a_u_id` = ?");
